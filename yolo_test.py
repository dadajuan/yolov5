...
class YOLO(object):
    _defaults = {
        ...
        # --------------------------------------------------------------------------#
        "model_path": 'model_data/yolov5_x.pth',
    ...
    def detect_image_cloud(self, image, crop=False):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
        image_shape = np.array(np.shape(image)[0:2])
        ...
        for i, c in list(enumerate(top_label)):
            ...
            label = label.encode('utf-8')
            print(label, top, left, bottom, right)
            if label.__contains__(b'laptop'):
                print('cmd start')
                cmd = 'ssh nvidia@172.17.17.52 "cd project;cd Tactical-helmet;cd lesson;python gpio.py"'
                os.system(cmd)
